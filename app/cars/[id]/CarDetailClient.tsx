...
export default function CarDetailClient({initialCar}:{initialCar:any}){
 const c=initialCar;
 const [active,setActive]=useState(0),[fullscreen,setFullscreen]=useState(false),[zoom,setZoom]=useState(1),[requestMode,setRequestMode]=useState<"enquiry"|"booking"|null>(null),[mediaView,setMediaView]=useState<any>(null),[mediaIndex,setMediaIndex]=useState(0);
 ...
 return <main className="carPage">
  ...
  {requestMode&&<VehicleEnquiryModal vehicle={c} source="vehicle_detail_page" intent={requestMode} onClose={()=>setRequestMode(null)}/>}
 </main>;
}

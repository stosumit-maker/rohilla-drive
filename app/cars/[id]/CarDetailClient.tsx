...
export default function CarDetailClient({initialCar}:{initialCar:any}){
 const c=initialCar;
 const [active,setActive]=useState(0),[fullscreen,setFullscreen]=useState(false),[zoom,setZoom]=useState(1),[enquire,setEnquire]=useState(false),[enquiryIntent,setEnquiryIntent]=useState<"enquiry"|"booking">("enquiry"),[mediaView,setMediaView]=useState<any>(null),[mediaIndex,setMediaIndex]=useState(0);
 ...
 return <main className="carPage">
  ...
  <section className="carDetail">
   <div className="photoColumn">
    ...
    {photos.length>1&&<p className="swipeHint">← Swipe photos • Tap photo to enlarge →</p>}{availableCategories.length>0&&<div className="vehicleInteractivePanel"><div><b>Explore this vehicle</b><small>Only real uploaded views are shown.</small></div><div className="vehicleViewButtons">{availableCategories.map(category=><button type="button" key={category} className={mediaView===category?"active":""} onClick={()=>{setMediaView(category);setMediaIndex(0)}}>{mediaLabels[category]}</button>)}</div>{activeMedia&&<div className={mediaView==="spin_360"?"vehicleMediaStage vehicleSpinStage":"vehicleMediaStage"} onPointerDown={spinPointerDown} onPointerMove={spinPointerMove} onPointerUp={spinPointerUp} onPointerCancel={spinPointerUp}>{activeMedia.media_kind==="video"?<video src={activeMedia.url} controls playsInline preload="metadata"/>:<img src={activeMedia.url} alt={mediaLabels[activeMedia.media_category]||"Vehicle view"} draggable={false}/>} {mediaView==="spin_360"&&selectedMedia.length>1&&<small className="vehicleSpinHint">Drag or swipe left / right to rotate • {mediaIndex+1} / {selectedMedia.length}</small>}{selectedMedia.length>1&&<div className="vehicleMediaPager"><button type="button" onClick={()=>setMediaIndex(i=>(i-1+selectedMedia.length)%selectedMedia.length)}>‹</button><span>{mediaIndex+1} / {selectedMedia.length}</span><button type="button" onClick={()=>setMediaIndex(i=>(i+1)%selectedMedia.length)}>›</button></div>}</div>}</div>}
   </div>
   <div className="carInfo">
    ...
    <div className="detailActionGrid"><button type="button" className="primary" onClick={()=>{track("Enquiry Open",{surface:"vehicle_detail",brand:c.brand,model:c.model});setEnquiryIntent("enquiry");setEnquire(true)}}>Enquire / WhatsApp</button><button type="button" className="bookNow" onClick={()=>{track("Booking Open",{surface:"vehicle_detail",brand:c.brand,model:c.model});setEnquiryIntent("booking");setEnquire(true)}}>Book Now</button></div>
  ...
  {enquire&&<VehicleEnquiryModal vehicle={c} source="vehicle_detail_page" intent={enquiryIntent} onClose={()=>setEnquire(false)}/>}
 </main>;
}

...
export default function AdminLayout({children}:{children:React.ReactNode}){
 ...
 async function enablePush(){
  try{
   ...
   if(permission!=="granted"){setNote("Notification access was not granted. Enable notifications in browser site settings and try again.");return}
   const reg=await navigator.serviceWorker.register("/admin-sw.js",{scope:"/"});
   ...
   setNote(test.error?`Notifications enabled; test failed: ${test.error.message}`:"Notifications enabled. Test alert sent.");
 ...
 return <div className="rdPortalScope rdAdminPortal">
  {ready&&<div className="rdAdminCommandBar" data-no-translate>
   <div className="rdCommandInner">
    <div className="rdCommandTitle"><span>Operations</span><strong>{total} pending</strong></div>
    <div className="rdCommandMetrics"><span>Sales {counts.sales}</span><span>Services {counts.services}</span><span>Dealer reviews {counts.dealers}</span><span>Partner reviews {counts.partners}</span><span>Verification {counts.verification}</span><span>Inventory review {counts.dealerVehicles}</span><span>Open deals {counts.dealRooms}</span></div>
    <div className="rdCommandActions">
    ...
    <div className="rdCommandNav">
     <a href="/admin">Executive Dashboard</a>
     <a href="/admin/add-vehicle">Inventory Management</a>
     <a href="/admin/revenue">Revenue & Settlements</a>
     <a href="/admin/new-vehicles">New Vehicle Leads</a>
     <a href="/admin/deal-rooms">Deal Management</a>
     <a href="/admin/finance">Transactions & RC</a>
     <a href="/admin/poster-scan">Poster Intelligence</a>
     <a href="/admin/vehicle-ai">Vehicle Intelligence</a>
     <a href="/admin/verification">Verification & Risk</a>
     <a href="/admin/growth">Marketing Centre</a>
     <a href="/admin/language">Language Operations</a>
     <a href="/admin/connections">Integrations</a>
     <a href="/business-hub">Partner Network</a>
     <a href="/inventory">Public Inventory</a>
     <a href="/new-vehicles">New Vehicle Experience</a>
     <a href="/">Public Website</a>
    </div>
    {note&&<small style={{width:"100%",color:"#b9c5d4"}}>{note}</small>}
   </div>
  </div>}
  {children}
 </div>;

...
export default function RevenuePage(){
 const db=supabase();
 const [ready,setReady]=useState(false),[msg,setMsg]=useState("Checking administrator access…"),[events,setEvents]=useState<Revenue[]>([]),[leads,setLeads]=useState<Lead[]>([]),[services,setServices]=useState<Service[]>([]),[vehicles,setVehicles]=useState<Vehicle[]>([]),[profiles,setProfiles]=useState<Profile[]>([]);
 const [form,setForm]=useState<any>({source_type:"used_vehicle",status:"potential"});
 useEffect(()=>{gate()},[]);
 async function gate(){const {data:{session}}=await db.auth.getSession();if(!session){setMsg("Administrator sign-in is required.");return}const {data:aal}=await db.auth.mfa.getAuthenticatorAssuranceLevel();if(aal?.currentLevel!=="aal2"){setMsg("Administrator authentication is required.");return}const {data:isAdmin}=await db.rpc("is_admin");if(!isAdmin){setMsg("Administrator access is required.");return}setReady(true);setMsg("");await load()}
 ...
 async function add(e:React.FormEvent){e.preventDefault();if(!form.title?.trim())return;setMsg("Saving revenue record…");const payload:any={source_type:form.source_type,title:form.title.trim(),status:form.status||"potential",notes:form.notes?.trim()||null,lead_id:form.lead_id||null,service_request_id:form.service_request_id||null,deal_room_id:form.deal_room_id||null,vehicle_id:form.vehicle_id||null,counterparty_id:form.counterparty_id||null,expected_fee:form.expected_fee?Number(form.expected_fee):null};const {error}=await db.from("revenue_events").insert(payload);if(error){setMsg(error.message);return}setForm({source_type:"used_vehicle",status:"potential"});setMsg("Revenue record saved successfully.");await load()}
 ...
 if(!ready)return <main className="section"><h1>Revenue & Collections</h1><p>{msg}</p><a className="call" href="/admin">Back to Dashboard</a></main>;
 return <main>
  <section className="section" style={{paddingTop:28}}><div className="head"><div><h1>Revenue & Collections</h1><p>Track potential, agreed, invoiced and received business revenue across vehicle, service and referral activity.</p></div><button onClick={load}>Refresh</button></div>
   <div className="stats-grid"><div className="stat-card"><strong>{kpi.count}</strong><span>Open Records</span></div><div className="stat-card"><strong>{money(kpi.potential)}</strong><span>Potential & Outstanding</span></div><div className="stat-card"><strong>{money(kpi.agreed)}</strong><span>Agreed & Invoiced</span></div><div className="stat-card"><strong>{money(kpi.received)}</strong><span>Received</span></div></div>
   <div className="card" style={{marginTop:18}}><div className="body"><h2>Add Revenue Record</h2><form className="adminForm" onSubmit={add}>
    ...
    <input required placeholder="Transaction / Opportunity Title" value={form.title||""} onChange={e=>setForm({...form,title:e.target.value})}/>
    <input type="number" min="0" step="1" placeholder="Expected ROHILLA DRIVE Fee (₹, Optional)" value={form.expected_fee||""} onChange={e=>setForm({...form,expected_fee:e.target.value})}/>
    ...
    <select value={form.counterparty_id||""} onChange={e=>setForm({...form,counterparty_id:e.target.value})}><option value="">Dealer / Partner (Optional)</option>{profiles.map(p=><option key={p.id} value={p.id}>{p.business_name||p.name||p.id} • {statusLabel(p.role||"")}</option>)}</select>
    <textarea placeholder="Commercial Notes / Next Action" value={form.notes||""} onChange={e=>setForm({...form,notes:e.target.value})}/>
    <button>Save Revenue Record</button>
   </form><p>{msg}</p></div></div>
  </section>
  <section className="section" style={{paddingTop:10}}><h2>Revenue Pipeline</h2>{events.length===0?<p>No revenue records have been created yet.</p>:<div className="request-list">{events.map((r,i)=><article className="request-card" key={r.id}><div className="request-top"><div><label>{sourceLabels[r.source_type]||statusLabel(r.source_type)}</label><h2>{r.title}</h2></div><span className={`status ${r.status}`}>{statusLabel(r.status)}</span></div><div className="row"><input type="number" min="0" placeholder="Expected (₹)" value={r.expected_fee??""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,expected_fee:e.target.value===""?null:Number(e.target.value)}:x))}/><input type="number" min="0" placeholder="Received (₹)" value={r.realized_fee??""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,realized_fee:e.target.value===""?null:Number(e.target.value)}:x))}/><select value={r.status} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,status:e.target.value}:x))}>{statuses.map(s=><option key={s} value={s}>{statusLabel(s)}</option>)}</select></div><textarea placeholder="Notes" value={r.notes||""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,notes:e.target.value}:x))}/><button onClick={()=>save(events[i])}>Save Changes</button></article>)}</div>}</section>
  <section className="section" style={{paddingTop:10}}><h2>Create Revenue Records from Existing Activity</h2><div className="grid"><div className="card"><div className="body"><h3>Recent Customer Leads</h3>{leads.slice(0,12).map(l=><div key={l.id} style={{padding:"9px 0",borderBottom:"1px solid #e5e7eb"}}><b>{l.customer_name}</b><small>{l.requirement||l.enquiry_type||"Vehicle enquiry"} • {statusLabel(l.status)}</small><button onClick={()=>prefillLead(l)}>Create Revenue Record</button></div>)}</div></div><div className="card"><div className="body"><h3>Recent Service Requests</h3>{services.slice(0,12).map(s=><div key={s.id} style={{padding:"9px 0",borderBottom:"1px solid #e5e7eb"}}><b>{s.customer_name}</b><small>{s.category} • {statusLabel(s.status)}</small><button onClick={()=>prefillService(s)}>Create Revenue Record</button></div>)}</div></div></div></section>
 </main>
}

...
const sourceLabels:any={used_vehicle:"Pre-Owned Vehicle Deal",new_vehicle:"New Vehicle Referral",inspection:"Inspection",service:"Service Revenue",finance_referral:"Finance Referral",insurance_referral:"Insurance Referral",transport:"Transport / Logistics",verification:"Verification",other:"Other"};
...
export default function RevenuePage(){
 const db=supabase();
 const [ready,setReady]=useState(false),[msg,setMsg]=useState("Verifying administrator access…"),[events,setEvents]=useState<Revenue[]>([]),[leads,setLeads]=useState<Lead[]>([]),[services,setServices]=useState<Service[]>([]),[vehicles,setVehicles]=useState<Vehicle[]>([]),[profiles,setProfiles]=useState<Profile[]>([]);
 const [form,setForm]=useState<any>({source_type:"used_vehicle",status:"potential"});
 useEffect(()=>{gate()},[]);
 async function gate(){const {data:{session}}=await db.auth.getSession();if(!session){setMsg("Administrator login required.");return}const {data:aal}=await db.auth.mfa.getAuthenticatorAssuranceLevel();if(aal?.currentLevel!=="aal2"){setMsg("Authenticator verification is required.");return}const {data:isAdmin}=await db.rpc("is_admin");if(!isAdmin){setMsg("Administrator access required.");return}setReady(true);setMsg("");await load()}
 ...
 async function add(e:React.FormEvent){e.preventDefault();if(!form.title?.trim())return;setMsg("Saving commercial opportunity…");const payload:any={source_type:form.source_type,title:form.title.trim(),status:form.status||"potential",notes:form.notes?.trim()||null,lead_id:form.lead_id||null,service_request_id:form.service_request_id||null,deal_room_id:form.deal_room_id||null,vehicle_id:form.vehicle_id||null,counterparty_id:form.counterparty_id||null,expected_fee:form.expected_fee?Number(form.expected_fee):null};const {error}=await db.from("revenue_events").insert(payload);if(error){setMsg(error.message);return}setForm({source_type:"used_vehicle",status:"potential"});setMsg("Commercial opportunity saved.");await load()}
 ...
 if(!ready)return <main className="section"><div className="portalSectionLabel">Commercial Operations</div><h1>Revenue Management</h1><p>{msg}</p><a className="call" href="/admin">Administration Console</a></main>;
 return <main>
  <section className="section" style={{paddingTop:28}}><div className="head"><div><div className="portalSectionLabel">Commercial Operations</div><h1>Revenue Management</h1><p>Track commercial opportunities, agreed fees, invoiced amounts and receipts across vehicle, service and referral workflows. No paid platform spend is required to use this ledger.</p></div><button onClick={load}>Refresh</button></div>
   ...
   <div className="card" style={{marginTop:18}}><div className="body"><h2>Add Commercial Opportunity</h2><form className="adminForm" onSubmit={add}>
    ...
    <input required placeholder="Opportunity / transaction title" value={form.title||""} onChange={e=>setForm({...form,title:e.target.value})}/>
    ...
    <button>Save Commercial Opportunity</button>
   </form><p>{msg}</p></div></div>
  </section>
  <section className="section" style={{paddingTop:10}}><div className="portalSectionLabel">Commercial Pipeline</div><h2>Revenue Opportunities</h2>{events.length===0?<p>No commercial opportunities recorded yet. Use an existing customer or service request below to create the first record.</p>:<div className="request-list">{events.map((r,i)=><article className="request-card" key={r.id}><div className="request-top"><div><label>{sourceLabels[r.source_type]||r.source_type}</label><h2>{r.title}</h2></div><span className={`status ${r.status}`}>{r.status}</span></div><div className="row"><input type="number" min="0" placeholder="Expected ₹" value={r.expected_fee??""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,expected_fee:e.target.value===""?null:Number(e.target.value)}:x))}/><input type="number" min="0" placeholder="Received ₹" value={r.realized_fee??""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,realized_fee:e.target.value===""?null:Number(e.target.value)}:x))}/><select value={r.status} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,status:e.target.value}:x))}>{statuses.map(s=><option key={s}>{s}</option>)}</select></div><textarea placeholder="Commercial notes" value={r.notes||""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,notes:e.target.value}:x))}/><button onClick={()=>save(events[i])}>Save Update</button></article>)}</div>}</section>
  <section className="section" style={{paddingTop:10}}><div className="portalSectionLabel">Demand Conversion</div><h2>Create Revenue Records from Existing Demand</h2><div className="grid"><div className="card"><div className="body"><h3>Recent Customer Leads</h3>{leads.slice(0,12).map(l=><div key={l.id} style={{padding:"9px 0",borderBottom:"1px solid #e5e7eb"}}><b>{l.customer_name}</b><small>{l.requirement||l.enquiry_type||"Vehicle enquiry"} • {l.status}</small><button onClick={()=>prefillLead(l)}>Create Revenue Record</button></div>)}</div></div><div className="card"><div className="body"><h3>Recent Service Requests</h3>{services.slice(0,12).map(s=><div key={s.id} style={{padding:"9px 0",borderBottom:"1px solid #e5e7eb"}}><b>{s.customer_name}</b><small>{s.category} • {s.status}</small><button onClick={()=>prefillService(s)}>Create Revenue Record</button></div>)}</div></div></div></section>
 </main>
}

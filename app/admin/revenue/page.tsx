...
export default function RevenuePage(){
 const db=supabase();
 const [ready,setReady]=useState(false),[msg,setMsg]=useState("Checking administration access…"),[events,setEvents]=useState<Revenue[]>([]),[leads,setLeads]=useState<Lead[]>([]),[services,setServices]=useState<Service[]>([]),[vehicles,setVehicles]=useState<Vehicle[]>([]),[profiles,setProfiles]=useState<Profile[]>([]);
 const [form,setForm]=useState<any>({source_type:"used_vehicle",status:"potential"});
 useEffect(()=>{gate()},[]);
 async function gate(){const {data:{session}}=await db.auth.getSession();if(!session){setMsg("Administration sign-in required.");return}const {data:aal}=await db.auth.mfa.getAuthenticatorAssuranceLevel();if(aal?.currentLevel!=="aal2"){setMsg("Administrator Authenticator verification is required.");return}const {data:isAdmin}=await db.rpc("is_admin");if(!isAdmin){setMsg("Administrator access required.");return}setReady(true);setMsg("");await load()}
 ...
 if(!ready)return <main className="section"><h1>Revenue & Collections</h1><p>{msg}</p><a className="call" href="/admin">Administration Dashboard</a></main>;
 return <main>
  <section className="section" style={{paddingTop:28}}><div className="head"><div><h1>Revenue & Collections</h1><p>Track potential, agreed, invoiced and received revenue across vehicle transactions, referrals and automotive services.</p></div><button onClick={load}>Refresh</button></div>
   ...
   <div className="card" style={{marginTop:18}}><div className="body"><h2>Create Revenue Record</h2><form className="adminForm" onSubmit={add}>
  ...
  <section className="section" style={{paddingTop:10}}><h2>Revenue Pipeline</h2>{events.length===0?<p>No revenue records have been logged yet.</p>:<div className="request-list">{events.map((r,i)=><article className="request-card" key={r.id}><div className="request-top"><div><label>{sourceLabels[r.source_type]||r.source_type}</label><h2>{r.title}</h2></div><span className={`status ${r.status}`}>{r.status}</span></div><div className="row"><input type="number" min="0" placeholder="Expected ₹" value={r.expected_fee??""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,expected_fee:e.target.value===""?null:Number(e.target.value)}:x))}/><input type="number" min="0" placeholder="Received ₹" value={r.realized_fee??""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,realized_fee:e.target.value===""?null:Number(e.target.value)}:x))}/><select value={r.status} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,status:e.target.value}:x))}>{statuses.map(s=><option key={s}>{s}</option>)}</select></div><textarea placeholder="Notes" value={r.notes||""} onChange={e=>setEvents(old=>old.map((x,n)=>n===i?{...x,notes:e.target.value}:x))}/><button onClick={()=>save(events[i])}>Save Update</button></article>)}</div>}</section>
  <section className="section" style={{paddingTop:10}}><h2>Convert Active Demand to Revenue</h2><div className="grid"><div className="card"><div className="body"><h3>Recent Customer Leads</h3>{leads.slice(0,12).map(l=><div key={l.id} style={{padding:"9px 0",borderBottom:"1px solid #e5e7eb"}}><b>{l.customer_name}</b><small>{l.requirement||l.enquiry_type||"Vehicle enquiry"} • {l.status}</small><button onClick={()=>prefillLead(l)}>Create Revenue Record</button></div>)}</div></div><div className="card"><div className="body"><h3>Recent Service Requests</h3>{services.slice(0,12).map(s=><div key={s.id} style={{padding:"9px 0",borderBottom:"1px solid #e5e7eb"}}><b>{s.customer_name}</b><small>{s.category} • {s.status}</small><button onClick={()=>prefillService(s)}>Create Revenue Record</button></div>)}</div></div></div></section>
 </main>
}

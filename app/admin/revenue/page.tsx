...
export default function RevenuePage(){
 ...
 useEffect(()=>{gate()},[]);
 async function gate(){const {data:{session}}=await db.auth.getSession();if(!session){setMsg("Administrator login required.");return}const {data:aal}=await db.auth.mfa.getAuthenticatorAssuranceLevel();if(aal?.currentLevel!=="aal2"){setMsg("Administrator authentication is required.");return}const {data:isAdmin}=await db.rpc("is_admin");if(!isAdmin){setMsg("Administrator access required.");return}setReady(true);setMsg("");await load()}
 ...
 if(!ready)return <main className="section"><h1>Revenue Management</h1><p>{msg}</p><a className="call" href="/admin">Back to Administration</a></main>;
 return <main>
  <section className="section" style={{paddingTop:28}}><div className="head"><div><h1>Revenue Management</h1><p>Track potential, agreed, invoiced and received business revenue across vehicle, service and referral activity.</p></div><button onClick={load}>Refresh</button></div>

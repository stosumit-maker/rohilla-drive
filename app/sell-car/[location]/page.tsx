...
export default async function RegionalSellCar({params}:{params:Promise<Params>}){
  ...
  return <main>
    ...
    <section className="hero" style={{paddingTop:52,paddingBottom:52}}><div className="heroText">
      ...
      <p className="heroSub">Callback • WhatsApp • Private photos</p>
      <p>{loc.marketNote} Start with a quick request or send complete vehicle details and photos.</p>
      <div className="row" style={{marginTop:18}}><a className="call" href="#ambala-enquiry">Request a Callback</a><a className="call" href="/sell">Add Details & Photos</a><a className="secondary" href={buyPath(loc)}>Buy Used Cars in {loc.name}</a></div>
    </div></section>

    <AmbalaLeadFunnel source={`regional_sell_${loc.slug}`} defaultMode="sell" defaultCity={loc.name} locationName={loc.name}/>

    <section className="section compactSection"><div className="head"><div><h2>How It Works</h2><p>Send basic details, then confirm the next step with us.</p></div></div><div className="grid">
    ...
    <section className="section"><div className="head"><div><h2>{loc.name} and Nearby Seller Coverage</h2><p>Nearby markets include {loc.nearby.join(", ")}. Rohilla Drive is based in Ambala City.</p></div></div>{related.length>0&&<div className="joinActions">{related.map(x=><a key={x.slug} href={sellPath(x)}>Sell car in {x.name}</a>)}</div>}</section>

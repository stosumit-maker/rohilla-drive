...
export default async function UsedCarsUnderFiveLakhAmbala(){
  ...
  return <main>
    ...
    {cars.length?<div className="grid">{cars.map(c=>{const photos=[...(c.vehicle_photos||[])].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));const first=photos[0]?.url;return <article className="card" key={c.id}><div className="photo real">{first?<img src={first} alt={`${c.year||""} ${c.brand} ${c.model} used car under 5 lakh in Ambala`}/>:<span>Vehicle photo unavailable</span>}</div><div className="body"><label>UNDER ₹5 LAKH • AMBALA</label><h2 style={{fontSize:22}}>{c.brand} {c.model}</h2>{c.variant&&<p>{c.variant}</p>}<small>{c.year||"Year on request"}{c.km!=null?` • ${Number(c.km).toLocaleString("en-IN")} km`:""}{c.fuel?` • ${c.fuel}`:""}{c.owner_count?` • ${c.owner_count} Owner`:""}</small>{c.asking_price!=null&&<strong>₹{Number(c.asking_price).toLocaleString("en-IN")}</strong>}<a className="call" href={`/cars/${c.id}`} style={{display:"block",textAlign:"center"}}>View Car Details</a></div></article>})}</div>:<div className="notice"><b>Budget fixed at ₹5 lakh?</b> <a href="#ambala-enquiry">Send the model and budget you need →</a></div>}
    </section>

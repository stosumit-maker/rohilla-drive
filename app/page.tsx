...
export default function Home(){
 ...
 function openVehicleBooking(car:Vehicle){setLeadType('vehicle booking');setLeadMessage(`I want to request booking for ${car.brand} ${car.model} ${car.variant||''} (${car.year}) listed at ₹${Number(car.asking_price).toLocaleString('en-IN')}. Vehicle ID: ${car.id}. Please confirm availability and booking terms.`);setLeadOpen(true)}
 ...
 async function submitLead(e:React.FormEvent<HTMLFormElement>){
  ...
  const isService=!['vehicle purchase','vehicle booking','vehicle sale','car purchase','car sale','new car purchase'].includes(leadType);
  if(isService){
  ...
  const wa=`${leadType==='vehicle booking'?'ROHILLA DRIVE BOOKING REQUEST':'ROHILLA DRIVE ENQUIRY'}\n\n${leadMessage}\n\nName: ${leadName}\nPhone: ${leadPhone}\nLocation: ${leadLocation}\n${leadTime?`Preferred time: ${leadTime}\n`:''}\nROHILLA DRIVE\n7015260003`;
 ...
 <section className="section inventorySection" id="inventory">
  ...
  {loading?<p>Loading cars...</p>:cars.length===0?<div className="card"><div className="body"><h3>Looking for a specific car?</h3><p>Tell us the model, budget and location. We will follow up with suitable options.</p><button onClick={()=>openLead("vehicle purchase")}>Send Requirement</button></div></div>:<div className="grid">{cars.map(car=>{const photos=[...(car.vehicle_photos||[])].sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));return <article className="card" key={car.id}><div className="photo real swipeGallery">{photos.length?photos.map((photo,index)=><img key={photo.url||index} src={photo.url} alt={`${car.brand} ${car.model} photo ${index+1}`} onClick={()=>window.location.href=`/cars/${car.id}`} title="Open vehicle details"/>):<PremiumIcon name="car"/>}</div>{photos.length>1&&<div className="swipeHint">← Swipe • Tap photo for details →</div>}<div className="body"><label>ROHILLA DRIVE</label><h3>{car.brand} {car.model}</h3>{car.variant&&<p>{car.variant}</p>}<small>{car.year} • {Number(car.km).toLocaleString("en-IN")} km • {car.fuel}{car.owner_count?` • ${car.owner_count} Owner`:""}</small><strong>₹{Number(car.asking_price).toLocaleString("en-IN")}</strong><a className="call" href={`/cars/${car.id}`} style={{display:"block",textAlign:"center",marginBottom:"6px"}}>View Details</a><div className="vehicleCardActions"><button onClick={()=>openVehicleEnquiry(car)}>Enquire</button><button className="bookNow" onClick={()=>openVehicleBooking(car)}>Book Now</button></div></div></article>})}</div>}
 </section>
 ...
 {leadOpen&&<div className="overlay"><div className="modal"><button className="x" onClick={resetLead}>×</button><h2>{leadType==='new car purchase'?'New Cars':leadType==='vehicle booking'?'Book This Car':leadType==='vehicle purchase'?'Find a Car':leadType==='vehicle sale'?'Sell Your Car':leadType==='Rohilla Trusted Assist'?'Rohilla Trusted Assist':`Request ${leadType}`}</h2><p>{leadType==='Rohilla Trusted Assist'?"Tell us what needs to be managed. We will confirm the requirement with you before any work proceeds.":['vehicle purchase','vehicle booking','vehicle sale','new car purchase'].includes(leadType)?(leadType==='new car purchase'?"Tell us the brand, model or budget you have in mind. We will help with available options.":leadType==='vehicle booking'?"Send a booking request. We will confirm availability and booking terms before anything is final.":"Tell us your requirement and our team will contact you."):"Your request is saved with Rohilla Drive and our team will coordinate the next step."}</p><form data-source="homepage_modal" onSubmit={submitLead}><input value={leadName} onChange={e=>setLeadName(e.target.value)} placeholder="Your name" required/><input value={leadPhone} onChange={e=>setLeadPhone(e.target.value)} placeholder="Mobile number" inputMode="tel" required/><input value={leadLocation} onChange={e=>setLeadLocation(e.target.value)} placeholder="City / Location" required/><input value={leadTime} onChange={e=>setLeadTime(e.target.value)} placeholder="Preferred time (optional)"/><textarea value={leadMessage} onChange={e=>setLeadMessage(e.target.value)} rows={5} required/><button type="submit" disabled={leadBusy}>{leadBusy?'Submitting…':leadType==='vehicle booking'?'Request Booking & Continue on WhatsApp':'Save Requirement & Continue on WhatsApp'}</button></form></div></div>}
 </main>
}

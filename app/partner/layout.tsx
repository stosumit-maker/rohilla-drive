 ...
 return <>
  ...
  <div data-no-translate style={{position:"fixed",left:12,bottom:14,zIndex:9998,display:"flex",flexDirection:"column",gap:8,alignItems:"flex-start",maxWidth:"calc(100vw - 24px)"}}>
   {open&&<div style={{width:"min(330px,calc(100vw - 24px))",maxHeight:"70vh",overflow:"auto",display:"grid",gap:7,padding:12,borderRadius:14,background:"rgba(255,255,255,.99)",border:"1px solid #d1d5db",boxShadow:"0 18px 55px rgba(0,0,0,.18)"}}>
    <div style={{padding:"2px 2px 7px",borderBottom:"1px solid #e5e7eb"}}><strong style={{fontSize:13,color:"#111827"}}>ROHILLA DRIVE Partner Portal</strong><div style={{fontSize:11,color:"#64748b",marginTop:2}}>Assigned work, deal coordination and business support</div></div>
    ...
    {path!=="/partner/deals"&&<a href="/partner/deals" style={pill}>Deal Management</a>}
    {path!=="/partner/growth"&&<a href="/partner/growth" style={pill}>Marketing Tools</a>}
    {path!=="/partner/language"&&<a href="/partner/language" style={pill}>Language Support</a>}
   ...
   <button onClick={()=>setOpen(x=>!x)} aria-expanded={open} style={{border:"1px solid #475569",background:"#0f172a",color:"#fff",borderRadius:10,padding:"10px 14px",fontWeight:800,boxShadow:"0 8px 24px rgba(0,0,0,.18)"}}>{open?"Close":"Partner Menu"}</button>
  </div>
 </>;
}

...
export default function PartnerPage(){
 ...
 if(session&&partner?.role==="partner"&&!partner.active)return <main><div className="auth"><h1>ROHILLA DRIVE</h1><h2>Partner Verification</h2><p>Your partner account is registered but is not yet active for operational assignments.</p><div className="notice">Complete your KYC and business documents first. After verification and approval, the full Partner Workspace will be enabled.</div><a className="call" href="/partner/kyc">Open KYC & Documents</a><button className="secondary" onClick={logout}>Sign Out</button><a href="/">Customer Website</a></div></main>;
 ...
 return <main><header><div className="brand"><b>ROHILLA DRIVE</b><small>Partner Workspace</small></div><button className="call" onClick={logout}>Sign Out</button></header>
  <section className="section"><div className="partner-head"><div><label>WORKSPACE OVERVIEW</label><h1>Welcome{partner?.name?`, ${partner.name}`:""}</h1><p>Manage assigned customer requests from quote through completion.</p></div><button className="secondary" onClick={loadRequests}>Refresh</button></div>{message&&<div className="notice">{message}</div>}
   <div className="grid" style={{margin:"14px 0"}}><article className="application"><h2>KYC & Documents</h2><p>Review your business verification status and upload required documents.</p><a className="call" href="/partner/kyc">Open KYC</a></article><article className="application"><h2>Deal Management</h2><p>Open deal workspaces specifically assigned to your business.</p><a className="call" href="/partner/deals">Open Deal Management</a></article><article className="application"><h2>Marketing Studio</h2><p>Prepare approved business content and trackable lead links.</p><a className="call" href="/partner/growth">Open Marketing Studio</a></article></div>

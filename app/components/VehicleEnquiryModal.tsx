...
export default function VehicleEnquiryModal({
  ...
  async function submit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const wa = [
      isBooking ? "ROHILLA DRIVE BOOKING REQUEST" : "ROHILLA DRIVE VEHICLE ENQUIRY",
      "",
      ...
      "",
      isBooking ? "Booking request saved. Booking is confirmed only after vehicle availability and booking amount are confirmed by ROHILLA DRIVE." : "This enquiry is saved with ROHILLA DRIVE.",
    ].filter(Boolean).join("\n");
  ...
  return (
    <div className="overlay" role="dialog" aria-modal="true" aria-label={intent==="booking"?"Vehicle booking request":"Vehicle enquiry"}>
      <div className="modal">
        ...
        <p>{intent==="booking"?"Send a booking request for this car. We will confirm availability and booking amount before the booking is final.":"Your enquiry will be saved first, then WhatsApp will open with the same vehicle details."}</p>

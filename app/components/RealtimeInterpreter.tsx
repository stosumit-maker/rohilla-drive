...
export default function RealtimeInterpreter({targetLanguage,label}:{targetLanguage:string;label:string}){
 ...
 async function start(){
  try{
   setNote("Connecting live interpretation…");setSource("");setTranslated("");
   const {data:{session}}=await db.auth.getSession();if(!session){setNote("Sign-in is required.");return}
   ...
   const clientSecret=secretJson?.client_secret?.value;if(!secretJson?.configured||!clientSecret){setNote(secretJson?.error||"Live interpretation service is not connected.");return}
   ...
   const events=pc.createDataChannel("oai-events");events.onmessage=({data})=>{try{const event=JSON.parse(data);if(event.type==="session.input_transcript.delta")setSource(x=>x+(event.delta||""));if(event.type==="session.output_transcript.delta")setTranslated(x=>x+(event.delta||""));if(event.type==="error")setNote(event.error?.message||"Live interpretation error.") }catch{}};
  ...
  }catch(err:any){stop();setNote(err?.message||"Live interpretation could not be started.")}
 }
 ...
 return <article className="application"><label>LIVE INTERPRETATION • {languageByCode(targetLanguage).nativeName}</label><h3>{label}</h3><p>Microphone audio is interpreted continuously while the session is active, and translated audio is played through this device.</p><div className="row">{active?<button onClick={stop}>Stop Interpretation</button>:<button onClick={start}>Start Live Interpretation</button>}<button onClick={()=>{setSource("");setTranslated("")}}>Clear Transcript</button></div>{note&&<div className="notice">{note}</div>}<p><b>Source Transcript:</b><br/>{source||"—"}</p><p><b>Translated Transcript:</b><br/>{translated||"—"}</p></article>
}

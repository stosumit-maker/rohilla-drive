...
export default function DealerJoinForm({kind}:{kind:"oem"|"preowned"}){
 const db=supabase();const [msg,setMsg]=useState("");const [busy,setBusy]=useState(false);
 const isOem=kind==="oem";const title=isOem?"OEM / Authorised New Vehicle Dealer":"Pre-Owned / Used Vehicle Dealer";
 const who=isOem
  ?"For OEM representatives, authorised showrooms, franchises and official new-vehicle sales teams. Add your business identity, authorised brands, territory and sales categories."
  :"For pre-owned dealerships handling regular stock, sourcing or buyer enquiries. Add your business details, main categories, operating city and approximate inventory. Individual owners selling one vehicle should use Sell / List.";
 const next=isOem

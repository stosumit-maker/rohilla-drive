...
export default function LanguageExperience(){
 const path=usePathname();const privatePortal=path.startsWith("/admin")||path.startsWith("/dealer")||path.startsWith("/partner");
 const [code,setCode]=useState("en-IN");const [show,setShow]=useState(false);const [note,setNote]=useState("");const busy=useRef(false);const debounce=useRef<number|undefined>(undefined);const language=useMemo(()=>languageByCode(code),[code]);
 useEffect(()=>{const saved=localStorage.getItem(LANGUAGE_STORAGE_KEY);if(saved){setCode(saved)}else setShow(true)},[]);
 useEffect(()=>{document.documentElement.lang=language.translationCode;window.dispatchEvent(new CustomEvent("rohilla-language-change",{detail:language}));if(code==="en-IN")return;if(privatePortal){setNote("Language saved. Use the secure Language Desk for customer conversations.");return}translateVisiblePage(code);const root=document.querySelector("main");if(!root)return;const observer=new MutationObserver(()=>{if(debounce.current)window.clearTimeout(debounce.current);debounce.current=window.setTimeout(()=>translateVisiblePage(code),450)});observer.observe(root,{childList:true,subtree:true});return()=>{observer.disconnect();if(debounce.current)window.clearTimeout(debounce.current)}},[code,path]);
 async function translateBatch(texts:string[],target:string){const r=await fetch("/api/translate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({texts,target,source:"en-IN"})});return await r.json()}
 async function translateVisiblePage(target:string){
  if(busy.current||privatePortal)return;busy.current=true;setNote("Translating…");
  try{
 ...
 return <>
  ...
  {note&&code!=="en-IN"&&<div data-no-translate style={{position:"fixed",right:12,top:112,zIndex:10019,maxWidth:260,fontSize:11,padding:"6px 9px",borderRadius:9,background:"rgba(17,24,39,.94)",color:"#fff"}}>{note}</div>}
  {show&&<div data-no-translate style={{position:"fixed",inset:0,zIndex:10050,background:"rgba(3,7,18,.82)",display:"grid",placeItems:"center",padding:18}}><div style={{width:"min(720px,100%)",maxHeight:"86vh",overflow:"auto",background:"#fff",color:"#111827",borderRadius:22,padding:20,boxShadow:"0 28px 80px rgba(0,0,0,.35)"}}><div style={{display:"flex",justifyContent:"space-between",gap:12,alignItems:"start"}}><div><b style={{fontSize:22}}>Choose your language</b><p style={{margin:"5px 0 16px"}}>अपनी भाषा चुनें • ਆਪਣੀ ਭਾਸ਼ਾ ਚੁਣੋ • ನಿಮ್ಮ ಭಾಷೆಯನ್ನು ಆಯ್ಕೆಮಾಡಿ</p></div>{typeof window!=="undefined"&&localStorage.getItem(LANGUAGE_STORAGE_KEY)&&<button onClick={()=>setShow(false)} style={{border:0,background:"transparent",fontSize:26}}>×</button>}</div><div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(145px,1fr))",gap:9}}>{ROHILLA_LANGUAGES.map(l=><button key={l.code} onClick={()=>choose(l.code)} style={{padding:"13px 10px",borderRadius:14,border:l.code===code?"2px solid #173326":"1px solid #d1d5db",background:l.code===code?"#eef7f1":"#fff",textAlign:"left"}}><b>{l.nativeName}</b><br/><small>{l.name}</small></button>)}</div><p style={{fontSize:12,color:"#4b5563",marginTop:16}}>Your choice is saved on this device. Public pages can be translated automatically when the translation connection is active. Secure portals keep customer/business data out of automatic page translation and use the Language Desk instead.</p></div></div>}
 </>
}
